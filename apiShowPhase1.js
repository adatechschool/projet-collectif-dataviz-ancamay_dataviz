const ajaxGet = (url, callback) => {
  // url, est l'URL de la ressource à récupérer, et callback, qui est une fonction à appeler une fois que la réponse est reçue.
  ...
  xhr.addEventListener("load", function () {
    //lorsque la rep est chargée, la fct anonyme appeler prq traiter rep
    if (xhr.status >= 1 && xhr.status < 250) {
      // compris entre 1-250
      ...
      console.error(
        "Erreur de chargement : " + xhr.status + " " + xhr.statusText
      ); // affiche msg erreur si c pas compris entre 1 et 250
    }
  });
  xhr.addEventListener("error", function () {
    // si la rqt échou pour qqconque rsn, elle sera appelé par la fonction anonyme
